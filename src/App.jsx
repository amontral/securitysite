...
export default function App() {
  ...
  return (
    <div style={{ backgroundColor: '#0c0c0e', color: 'white', minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif" }}>
      {/* Top Navigation */}
      <div style={{ backgroundColor: '#f8f9fa', color: '#0c0c0e', borderBottom: '1px solid #ccc', padding: '1rem', position: 'fixed', width: '100%', zIndex: 1000 }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: '1.2rem' }}>Silex Strategic Group</h2>
          <div onClick={() => setMenuOpen(!menuOpen)} style={{ cursor: 'pointer', fontSize: '1.5rem', marginRight: '1rem' }}>{menuOpen ? '✕' : '☰'}</div>
        </div>
        {menuOpen && (
      ...
      {/* Hero Section */}
      <div style={{ paddingTop: '7rem', textAlign: 'center', backgroundColor: '#1f2a35', padding: '4rem 1rem 2rem' }}>
        <h1 style={{ fontSize: '4rem', fontWeight: '900', textShadow: '0 0 10px rgba(255,255,255,0.9)' }}>Silex Strategic Group</h1>
        <p style={{ color: '#aaa', fontSize: '1.2rem' }}>Strategic Security. Real-World Results.</p>
        <img src="/sbss-badge.png" alt="SBSS Badge" style={{ marginTop: '2rem', opacity: 0.25, width: '180px', filter: 'drop-shadow(0 0 45px rgba(255, 255, 255, 0.6))' }} />
      </div>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem 1rem' }}>
        ...
        <section style={sectionStyle}>
          ...
          <p style={paragraph}>The Small Business Security Standard (SBSS) simplifies enterprise-grade risk principles into actionable controls for small businesses.</p>
        ...
        <section style={sectionStyle}>
          ...
          <p style={paragraph}>A cyberattack may start with a stolen keycard or an untrained employee. We advocate layered, end-to-end protection—from locked doors to encrypted networks.</p>
        </section>
      ...
      {showDisclaimer && (
        <div style={modalOverlay}>
          <div style={modalBox}>
            <button onClick={handleCloseDisclaimer} style={closeButton}>×</button>
            <h2>Before You Begin</h2>
            <p style={{ color: '#ccc' }}>The Small Business Security Assessment is a self-evaluation tool. By proceeding, you agree that:</p>
            <ul style={{ color: '#ccc', fontSize: '0.85rem', textAlign: 'left' }}>
            ...
            <form onSubmit={handleDisclaimerSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              <label><input type="checkbox" checked={formData.agreed} onChange={e => setFormData({ ...formData, agreed: e.target.checked })} /> I agree to the disclaimer terms</label>
